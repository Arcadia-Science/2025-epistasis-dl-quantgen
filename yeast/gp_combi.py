...
base_file_name_out = 'experiments/BYxRM_gpcombi_scaled'
...
loaders = gpatlas.create_data_loaders(base_file_name, batch_size=batch_size, num_workers=3, shuffle=True)
        ...
        if test_loader is not None:
            ...
            with torch.no_grad():
                for phens, gens in test_loader:
                    ...
                    output = model(gens)
                    test_loss += F.mse_loss(phens_add + output + EPS, phens)
