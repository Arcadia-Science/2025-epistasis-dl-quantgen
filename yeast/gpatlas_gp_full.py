...
#####################################################################################################################
#####################################################################################################################
#simple gen encoder for gpatlas a la Dave
class GQ_net(nn.Module):
    ...
    """
    def __init__(self, n_loci=n_loci*n_alleles, N=2048, latent_dim=32, batchnorm_momentum=batchnorm_momentum):
        super().__init__()
   ...
   # Create mask for phenotype selection
    ...
    for epoch_gp in range(num_epochs_gp):
        ...
        for i, (phens, gens) in enumerate(train_loader):
            ...
            g_p_recon_loss = g_p_recon_loss + l1_reg * weights_regularization_l1 + l2_reg * weights_regularization_l2
        ...
        if epoch_gp % 1 == 0:
            ...
            with torch.no_grad():
                for phens, gens in test_loader:
                    ...
                    test_loss = F.mse_loss(X_selected + EPS, phens_selected + EPS)
            ...
            if avg_test_loss_gp < (best_loss_gp - min_delta):
                ...
                best_model_state = {k: v.cpu().detach().clone() for k, v in GQ.state_dict().items()}
...
def run_full_pipeline():
    ...
    GQ = GQ_net(latent_dim=latent_space_p, N=latent_space_gp).to(device)

    best_loss_gp, GQ = train_gp_model(GQ,
                         P=P,
                         ...
                         n_alleles=n_alleles,
                         gen_noise=0.8,
                         weights_regularization_l1=0.8,
                         weights_regularization_l2=0.01)
