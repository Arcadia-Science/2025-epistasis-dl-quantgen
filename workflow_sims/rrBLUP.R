...
output_corr_summary <- snakemake@output[["correlation_summary"]]
output_pred_pheno_output <- snakemake@output[["pred_pheno_output"]]
output_pheno_pred_plot <- snakemake@output[["pred_pheno_output_plot"]]



#######################
#df <- read.table('../alphasimr/WF_Ne100k_samples_10k_H2_05_qtl_10k_corA_02_25traits_g.txt', header = T)
df <- read.table(input_geno, header = T)
...
pheno <- read.table(input_pheno, header = T)
...
loci_eff <- read.table(loci_effects, header = T)
...
markers_test = markers[-c(1:n_train),]

pheno_train = pheno[1:n_train,]
pheno_test = pheno[-c(1:n_train),]
...
fit_lambda <- mod_trait_train$Ve/mod_trait_train$Vu
...
write.table(mod_cors, output_corr_summary, quote = F, col.names = T, row.names = F)
...
write.table(pheno_output, output_pred_pheno_output, quote = F, col.names = T, row.names = F)
...
pl_pheno_cor <- ggplot(pheno_output, aes(x=true_pheno, y=pred_pheno, colour = as.factor(rep))) + geom_point() +
                facet_wrap(~epistasis_bin) + theme_bw() + theme(legend.position="none")
