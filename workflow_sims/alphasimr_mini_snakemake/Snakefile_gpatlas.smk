...
rule all:
   input:
        ...
        #get_valid_outputs('gpnet/qhaplo_{qtl_n}qtl_{sample_size}n_rep{rep}_phenotype_correlations.csv'),
        ...
        #get_valid_outputs("gplinear/qhaplo_{qtl_n}qtl_{sample_size}n_rep{rep}_phenotype_correlations.csv")
...
rule optimize_fit_gpnet:
    ...
    output:
        pheno_corrs = 'gpnet/qhaplo_{qtl_n}qtl_{sample_size}n_rep{rep}_phenotype_correlations.csv'
    resources:
    params:
...
rule optimize_fit_gplinear:
    ...
    input:
        ...
        input_test_data = rules.generate_input_data.output.test_data_input,
    output:
        pheno_corrs = 'gplinear/qhaplo_{qtl_n}qtl_{sample_size}n_rep{rep}_phenotype_correlations.csv'
    resources:
    params:
        sample_size = "{sample_size}",
        qtl_n = "{qtl_n}",
