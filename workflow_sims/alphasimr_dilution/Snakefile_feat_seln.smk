...
rule all:
   input:
        expand('gphybrid/qhaplo_{qtl_n}qtl_{marker_n}marker_{sample_size}n_mlp_pruned_correlations.csv', qtl_n=QTL_N, marker_n=MARKER_N, sample_size=SAMPLE_SIZE),
...
rule optimize_fit_gpnet:
    ...
    output:
        pheno_corrs = 'gphybrid/qhaplo_{qtl_n}qtl_{marker_n}marker_{sample_size}n_mlp_pruned_correlations.csv'
    resources:
    params:
