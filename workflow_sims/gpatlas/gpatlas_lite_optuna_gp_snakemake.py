...
##########################################################################################
##########################################################################################
train_data_gp = GenoPhenoDataset(snakemake.input['input_train_data'])
test_data_gp = GenoPhenoDataset(snakemake.input['input_test_data'])

train_data_pheno = PhenoDataset(snakemake.input['input_train_data'])
test_data_pheno = PhenoDataset(snakemake.input['input_test_data'])
...
checkpoint = torch.load(snakemake.input['gg_encoder_optimized'])
...
def train_final_model(best_params, train_loader_p, train_loader_gp, test_loader_p, test_loader_gp,
    ...
    pp_save_path = snakemake.output['pp_encoder_optimized']
    gp_save_path = snakemake.output['gp_encoder_optimized']
...
def main():
    ...
    output_dir = Path(snakemake.output['optuna_gp_csv']).parent
    ...
    try:
        ...
        results_df.to_csv(snakemake.output['optuna_gp_csv'], index=False)
        ...
        with open(snakemake.output['optuna_gp_json'], 'w') as f:
            json.dump(study_info, f, indent=4)

        print(f"\nResults saved to:")
        print(snakemake.output['optuna_gp_csv'])
        print(snakemake.output['optuna_gp_json'])
        ...
        error_log = Path(snakemake.output['optuna_gp_csv']).parent / f'optuna_error_gp_{timestamp}.txt'
